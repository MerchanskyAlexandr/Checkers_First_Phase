...
namespace Checkers.GameEngine
{
    public class Desk
    {
        ...
        public const byte THE_BOARDSIZE = 8;

        #endregion

        #region Private Fields

        private int[,] _cells = new int[THE_BOARDSIZE, THE_BOARDSIZE];
        ...
        public Desk()
        {
            for (int i = 0; i < THE_BOARDSIZE; i++)
            {
                for (int j = 0; j < THE_BOARDSIZE; j++)
                {
                    _cells[i, j] = 0;
                }
            }
        ...
        public CheckerOnDesk LastRemoteChecker { get; set; }
        ...
        public void PossibleChangeOnKing(CheckerOnDesk chD, Point point)
        {
            ...
            if (point.X == THE_BOARDSIZE - 1 && chD.Color == ColorType.White && chD.Status == CheckerStatus.Simple)
            {
        ...
        public List<Point> IsToSimpleBit(CheckerOnDesk chD)
        {
            ...
            if (xx + 2 < THE_BOARDSIZE && yy + 2 < THE_BOARDSIZE && IsEnemy(chD, new Point(xx + 1, yy + 1)) && IsEmpty(new Point(xx + 2, yy + 2)))
            {
                points.Add(new Point(xx + 2, yy + 2));
            }

            if (xx - 2 >= 0 && yy + 2 < THE_BOARDSIZE && IsEnemy(chD, new Point(xx - 1, yy + 1)) && IsEmpty(new Point(xx - 2, yy + 2)))
            {
                points.Add(new Point(xx - 2, yy + 2));
            }
            ...
            if (xx + 2 < THE_BOARDSIZE && yy - 2 >= 0 && IsEnemy(chD, new Point(xx + 1, yy - 1)) && IsEmpty(new Point(xx + 2, yy - 2)))
            {
                points.Add(new Point(xx + 2, yy - 2));
            }
        ...
        public List<Point> IsToKingBit(CheckerOnDesk chD)
        {
            ...
            int x = chD.Point.X;
            int y = chD.Point.Y;
            int xx, yy, enemyCount;

            xx = x + 1;
            yy = y + 1;
            enemyCount = 0;
            while (xx < THE_BOARDSIZE - 1 && yy < THE_BOARDSIZE - 1)
            { 
                Point point = new Point(xx, yy);
                bool enemy = IsEnemy(chD, point);
                if (!enemy && !IsEmpty(point))
                {
                    break;
                }
                if (enemy)
                ...
                if (enemy && IsEmpty(new Point(xx + 1, yy + 1)))
                {
        ...
        public List<Point> IsToSimpleMove(CheckerOnDesk chD)
        {
            ...
            if (x + i < THE_BOARDSIZE && y + 1 < THE_BOARDSIZE && (_cells[x + i, y + 1] == 0))
            {
                points.Add(new Point(x + i, y + 1));
            }
            if (x + i < THE_BOARDSIZE && y - 1 >= 0 && (_cells[x + i, y - 1] == 0))
            {
                points.Add(new Point(x + i, y - 1));
            }
        ...
        public void RemoveChecker(Point p1, Point p2)
        {
            ...
            LastRemoteChecker = GetCheckerOnDesk(p);
        ...
        public void MoveCheking(Point p1, Point p2, out bool isBeat, ColorType color)
        {
            ...
            if (IsToBit(chD).Count == 0)  
            {
                if (IsToMove(chD).Contains(p2)) 
                {
                    ...
                    CheckerOnDesk chDnew = new CheckerOnDesk(chD.Color, chD.Status, p2);
                    _checkersOnDesk.Add(chDnew);
                    PossibleChangeOnKing(chDnew, p2);
        ...
        private List<Point> DiagonalForwardRight(Point point)
        {
            ...
            int xx = point.X + 1;
            int yy = point.Y + 1;
            while (xx < THE_BOARDSIZE && yy < THE_BOARDSIZE)
            {
                if (_cells[xx, yy] == 0)
                {
                    points.Add(new Point(xx, yy));
