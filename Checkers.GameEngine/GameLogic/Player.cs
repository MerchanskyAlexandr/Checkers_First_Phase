...
namespace Checkers.GameEngine
{
    public class Player
    {
        ...
        public CheckerOnDesk MovingChecker
        {
            get
            {
                return _movingChecker;
            }
            set
            {
        ...
        private void FirstClick(Point firstPoint, out bool isThisFirstClick)
        {
            ...
            if (_desk.GetCheckerOnDesk(firstPoint) != null) 
            {
                ...
                {
                    if (_desk.IsToBit(checker).Count != 0) 
                    {
        ...
        private void SecondClick(Point p, out bool isCangeChecker, out bool wasBeat)
        {
            ...
            if (_moveStarted && !LightPoints.Contains(p) && LightPoints.Count != 0)
            {
                isCangeChecker = true;;
                return;
            }

            if (!LightPoints.Contains(p) && LightPoints.Count != 0)
            {
                if (!_desk.IsEmpty(p) && _desk.GetCheckerOnDesk(p).Color == this.Color)
                {
                    ...
                    {
                        if (_desk.IsToBit(_desk.GetCheckerOnDesk(p)).Count != 0)  
                        {
                            _isCorrect = true;
                            _endMove = false;
                            isCangeChecker = true;
